...
/// Bytes in use out of a buffer's blocks. Debug reporting only.
...
public:
    ...
    /**
     * Creates the upload buffer and the first block of every fixed-size kind.
     * A growable kind takes its first block on its first allocation.
     *
     ...
     * @return an owning array; empty only when the allocation failed.
     */
    ...
    [[nodiscard]] GpuPtr<StaticRecord<Kind>> staticBase() const {
        static_assert(!StaticBufferTraits<Kind>::growable,
                      "a growable kind spans several blocks and has no single "
                      "base address");
        return GpuPtr<StaticRecord<Kind>>{staticBaseAddress(Kind).address};
    }

    /// @return name, total block bytes, bytes in use and block count.
...
private:
    ...
    /// @return the appended block, or nullptr when it could not be created.
    [[nodiscard]] StaticBuffer* addBlock(StaticBufferKind kind, vk::DeviceSize bytes);

    /// @return the block holding `address`, or nullptr when none does.
    [[nodiscard]] StaticBuffer* blockOf(StaticBufferKind kind,
                                        GpuPtr<std::byte> address);

    [[nodiscard]] GpuPtr<std::byte> staticBaseAddress(StaticBufferKind kind) const;

    /// One entry per kind, in allocation order. A block is never destroyed alone.
    std::array<std::vector<StaticBuffer>, STATIC_BUFFER_COUNT> staticBuffers_{};
